---
name: Fetch an Acko two-wheeler insurance certificate
description: Retrieve a citizen's Acko two-wheeler (bike) insurance policy certificate through India's API Setu / DigiLocker platform.
api: openapi/acko-apisetu-openapi-original.json
operations: [twipc]
---

# Fetch an Acko two-wheeler insurance certificate

Use this skill to pull an Acko two-wheeler (bike) insurance policy certificate for
a consenting citizen via the government API Setu gateway.
...
2. `POST /twipc/certificate` (operationId `twipc`) with the JSON request body and
